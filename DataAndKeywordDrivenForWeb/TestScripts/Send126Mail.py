...
from TestScripts.CreateContacts import createContact
import traceback


def createContactAndSend126Mail():
    try:
        excelObj = ParseExcel()
        ...
        for idx, i in enumerate(isExecuteCaseCols[1:]):
            ...
            if i.value == "y":
                ...
                if frameworkName == "关键字":
                    ...
                    for j in range(2, stepNum + 1):
                        ...
                        try:
                            ...
                            writeResult(excelObj, stepSheet, "Faild", j, "caseStep", errMsg=errMsg,
                    ...
                    if successfulStepNum == stepNum - 1:
                        ...
                        logger.info("用例“{}”执行成功".format(caseName))
                    else:
                        writeResult(excelObj, caseSheet, "Faild", idx + 2, "testCase")
                        logger.info("用例“{}”执行失败".format(caseName))
                    ...
                    result = createContact(excelObj, stepSheet, dataSheet)
                    if result:
                        successfulCaseNum += 1
                        writeResult(excelObj, caseSheet, "Pass", idx + 2, "testCase")
                        logger.info("用例“{}”执行成功".format(caseName))
                ...
                logger.info("用例“{}”被设置为忽略执行".format(caseName))
        logger.info("共{}条用例，{}条需要被执行，本次执行通过{}条".format(caseNum - 1, requiredCaseNum, successfulCaseNum))
    except Exception as e:
